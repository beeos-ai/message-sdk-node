 ...
 * shared by the web, desktop and React Native platforms. `createMessageClient`
 * consumes this shape directly; `createGatewayMessageClientComposition` is
 * never part of a public package entrypoint.
 */
export interface GatewayMessageClientOptions {
  ...
  /**
   * Optional on "web"/"desktop": those platforms authenticate primarily via
   * the same-origin session cookie (`credentials: "include"`) and only add a
   * Bearer token when one is supplied and non-empty. Required in practice on
...
/**
 * Private Gateway/v1 composition with one hidden Centrifugo WSS, shared by
 * every non-Node platform. Not exported from any package entrypoint —
 * `createMessageClient(options: GatewayMessageClientOptions)` and the
 * deprecated `createReactNativeMessageClientComposition` are the only
 * callers.
 */
export function createGatewayMessageClientComposition(
  options: GatewayMessageClientOptions,
): MessageClientComposition {
  ...
  return {
    ...
    messageStream: http,
    runtimeMethods: http,
    realtime: withPersonalNotificationNormalization(new CentrifugoSessionAdapter({
...
class GatewayCredentials implements CurrentPrincipalPort {
  ...
  constructor(private readonly options: GatewayMessageClientOptions) {}

  async accessToken(): Promise<string | undefined> {
    if (!this.options.accessTokenProvider) {
      if (this.options.platform === "mobile") {
        throw new Error("Gateway access token provider is required for platform \"mobile\"");
      }
      return undefined;
    }
    ...
    if (!trimmed) {
      if (this.options.platform === "mobile") {
        throw new Error("Gateway access token provider returned an empty token");
      }
      return undefined;
    }
    return trimmed;
...
class GatewayHttpAdapter {
  ...
  constructor(
    private readonly options: GatewayMessageClientOptions,
...
async function requestResponse(
  ...
  const response = await fetchImpl(`${normalizeBase(options.gatewayUrl)}${path}`, {
    method,
    headers,
    // Web/desktop rely on the same-origin session cookie; mobile has no
    // cookie jar and never sent this field, so its behavior is unchanged.
...
function isRealtimeEnvelopeShaped(
  value: unknown,
): value is Record<string, unknown> & { type: string; eventId: string; scope: Record<string, unknown> } {
  if (typeof value !== "object" || value === null || Array.isArray(value)) return false;
  const raw = value as Record<string, unknown>;
  return typeof raw.type === "string"
    && typeof raw.eventId === "string"
    && typeof raw.scope === "object" && raw.scope !== null && !Array.isArray(raw.scope);
}

/**
 * The Gateway personal channel may deliver a thin, envelope-less signal
 * instead of a fully-typed RealtimeEventV1 (the "thin pass-through" realtime
 * design). Anything not already a well-formed envelope is normalized into the
 * canonical `personal.notification` wake signal so it still flows through
 * the one MessageClient.listen() pipeline and the existing HTTP-recovery
 * boundary — no new cursor/checkpoint/epoch is introduced; a fully-formed
 * envelope (e.g. message.created, inbox.conversation.available) always
 * passes through unchanged.
 */
function normalizePersonalNotification(raw: unknown): unknown {
  if (isRealtimeEnvelopeShaped(raw)) return raw;
  ...
  const eventId = typeof record.eventId === "string" && record.eventId
    ...
    : typeof record.id === "string" && record.id
      ? record.id
      : `personal-notification:${++notificationSequence}`;
  return {
    schemaVersion: 1,
    eventId,
    type: "personal.notification",
    scope: { tenantId: "", ...(conversationId ? { conversationId } : {}) },
    actor: { kind: "service", id: "gateway" },
    correlation: {},
    occurredAt: new Date().toISOString(),
    data: { ...(conversationId ? { conversationId } : {}), raw: record },
  };
}

function withPersonalNotificationNormalization(inner: RealtimeSessionPort): RealtimeSessionPort {
  return {
    connect: (input: RealtimeConnectInput) => inner.connect({
      onState: input.onState,
      onEvent: (raw) => input.onEvent(normalizePersonalNotification(raw)),
    }),
  };
}
...
function normalizeBase(value: string): string {
  ...
  if (url.protocol !== "https:" && !isLoopback(url)) {
    throw new Error("Gateway composition requires HTTPS");
  }
  return base;
}
