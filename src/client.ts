...
export class MessageClient extends EventEmitter {
  ...
  private readonly opts: MessageClientOptions;
  ...
  constructor(opts: MessageClientOptions) {
    super();
    if (!opts.serviceUrl) throw new Error("message-sdk: serviceUrl is required");
...
export class ConversationsAPI {
  ...
  async subscribe(conversationId: string): Promise<ConversationSubscription> {
    ...
    return new ConversationSubscription(cent, conversationId);
  }
}
...
export class ConversationSubscription extends EventEmitter {
  ...
  constructor(centrifuge: Centrifuge, public readonly conversationId: string) {
    super();
    this.sub = centrifuge.newSubscription(`ch:${conversationId}`);
    this.sub.on("subscribed", () => this.emit("subscribed"));
    this.sub.on("unsubscribed", () => this.emit("unsubscribed"));
    this.sub.on("publication", (ctx: PublicationContext) => {
      try {
        const msg = envelopeToMessage(ctx.data as WireEnvelope);
        this.emit("message", msg);
      } catch (err) {
        this.emit("error", err instanceof Error ? err : new Error(String(err)));
      }
